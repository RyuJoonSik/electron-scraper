...
async function 엑셀_워크시트_목업_생성() {
  const 워크북 = 워크북_생성();
  const 엑셀_파일_경로 = 'data/Products.xlsx';
  const 가져온_워크북 = await 읽은_엑셀_파일_워크북_생성(워크북, 엑셀_파일_경로);
  const 워크_시트_이름 = 'My Products';
  const 워크_시트 = 워크_시트_추출(가져온_워크북, 워크_시트_이름);
...
describe('워크시트_칼럼_배열_생성', () => {
  it('칼럼 배열을 받아 워크 시트 칼럼 객체 배열을 반환한다.', () => {
    const 칼럼_배열 = ['제품 ID', '이름'];
    const 워크시트_칼럼_배열 = 워크시트_칼럼_배열_생성(칼럼_배열);
    const [첫번째_칼럼, 두번째_칼럼] = 워크시트_칼럼_배열;
    const 예상_첫번째_칼럼 = {
      header: '제품 ID',
      key: '제품_ID'
    };
...
describe('워크시트_제품_칼럼_배열_생성', () => {
  it('제품 칼럼 배열을 받아 워크 시트 제품 칼럼 객체 배열을 반환한다.', () => {
    const 워크시트_제품_칼럼_배열 = 워크시트_제품_칼럼_배열_생성();
    const 예상_배열 = [
...
describe('워크시트_생성', () => {
  it('워크 북에 워크 시트를 추가한다.', () => {
    ...
    const 워크북_스파이 = jest.spyOn(워크북, 'addWorksheet');
    const 워크시트 = 워크시트_생성(워크북);

    expect(워크북_스파이).toBeCalledWith('My Products');
    expect(워크시트).toBeTruthy();
  });
});
...
describe('워크시트_설정', () => {
  it('워크 북의 워크 시트 칼럼들을 설정한다.', () => {
    ...
    const 워크북 = new ExcelJS.Workbook();
    const 워크시트 = 워크시트_생성(워크북);
    const 워크시트_스파이 = jest.spyOn(워크시트, 'addRow');

    워크시트_설정(워크시트, 제품_정보_배열);

    const 칼럼_배열 = Object.keys(제품);
    const {columns: 워크시트_칼럼_배열} = 워크시트;

    워크시트_칼럼_배열.forEach((워크시트_칼럼, 인덱스) => {
      expect(워크시트_칼럼['_key']).toBe(칼럼_배열[인덱스]);
    });
    expect(워크시트_스파이).toBeCalledTimes(1);
  });
});
...
describe('워크북_생성', () => {
  ...
  expect(워크북).toHaveProperty('xlsx');
  expect(워크북.xlsx).toHaveProperty('writeFile');
  expect(워크북.xlsx).toHaveProperty('readFile');
});

describe('읽은_엑셀_파일_워크북_생성', () => {
  it('해당 경로의 엑셀 파일을 읽어와 워크북 객체로 반환한다.', async () => {
    const 워크북 = 워크북_생성();
    const 임시_워크북 = 워크북_생성();
    const readFile_스파이 = jest.spyOn(워크북.xlsx, 'readFile').mockImplementation(() => 임시_워크북);
    const 엑셀_파일_경로 = 엑셀_파일_경로_생성();
    const 제품_워크북 = await 읽은_엑셀_파일_워크북_생성(워크북, 엑셀_파일_경로);

    expect(readFile_스파이).toBeCalledWith(엑셀_파일_경로);
    expect(제품_워크북).toEqual(임시_워크북);
  });
});

describe('워크_시트_추출', () => {
  it('워크북에서 해당 이름의 워크시트를 추출한다.', () => {
    const 워크북 = 워크북_생성();
    const 임시_워크_시트 = 워크시트_생성(워크북);
    const getWorksheet_스파이 = jest.spyOn(워크북, 'getWorksheet').mockImplementation(() => 임시_워크_시트);
    const 워크_시트 = 워크_시트_추출(워크북);

    expect(getWorksheet_스파이).toBeCalledWith('My Products');
    expect(워크_시트).toEqual(임시_워크_시트);
  });
});
