...
export default class 검색바_브랜드 extends 컴포넌트 {
  브랜드_사전: { [key: string]: string } = 브랜드;

  constructor(부모_컴포넌트: HTMLElement, 속성: { [property: string]: any }) {
    super(부모_컴포넌트, 속성);
  ...
  변경_이벤트(e: Event) {
    const 새_검색어 = (e.target as HTMLSelectElement).value;

    this.상태_설정({ 검색어: 새_검색어 });
  }

  설정() {
    const 셀렉트 = this.부모_컴포넌트.querySelector('[data-testid="search-bar-select"]') as HTMLSelectElement;
