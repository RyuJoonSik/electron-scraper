...
export default class 다운로드_바 extends 컴포넌트 {
  ...
  설정() {
    const 다운로드_버튼 = this.부모_컴포넌트.querySelector('[data-testid="download-bar-button"]') as HTMLButtonElement;
  ...
  async 클릭_이벤트() {
    const { 다운로드 } = this.속성;
    const { value: 시작_페이지_번호 } = this.부모_컴포넌트.querySelector(
      '[data-testid="download-bar-start-page"]'
    ) as HTMLInputElement;
    const { value: 끝_페이지_번호 } = this.부모_컴포넌트.querySelector(
      '[data-testid="download-bar-end-page"]'
    ) as HTMLInputElement;
